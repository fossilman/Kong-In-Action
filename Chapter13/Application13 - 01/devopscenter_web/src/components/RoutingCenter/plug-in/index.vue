...
import { saveOrUpdate } from "@/server/routeplus";
@Component({
  components: {
    rateLimiting
  }
})
export default class RouterPlug extends Vue {
  ...
  public openRoutring(
    ...
    this.gatewayType.name = gatewayName;
    this.gatewayType.type = gatewayType;
        ...
        (res: any) => {
          ...
          saveOrUpdate(data).then(
            (resId: any) => {
              ...
              this.$message.success(resId.errmsg);
              this.$emit("updatePlug", data, resId.retval.id);
            },
            (err: any) => {

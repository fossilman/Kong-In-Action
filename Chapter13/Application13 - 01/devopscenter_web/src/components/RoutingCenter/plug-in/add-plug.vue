...
export default class RouterPlugList extends Vue {
  ...
  public openRoutring(row: any): void {
    this.routeData = {
      kongRoutesName: row.kongServicesName || row.kongRoutesName,
      kongServiceName: row.kongServicesName || row.kongRoutesName,
      pluginAttr: {
        quantity: 0,
        period: ""
      },
  ...
  private handleSave(): void {
    if (this.routeData.pluginName === "rate-limiting") {
      (this.$refs[this.routeData.pluginName] as rateLimiting).getData().then(
        (res: any) => {
          this.routeData.pluginAttr = res;
          const data: any = this.routeData;
          ...
          this.$emit("updatePlug", {
            kongRoutesName: data.kongRoutesName,
            kongServiceName: data.kongServiceName,
            pluginId: data.pluginId,
            pluginConfig: {
              [data.pluginAttr.period]: data.pluginAttr.quantity
            }
          });
